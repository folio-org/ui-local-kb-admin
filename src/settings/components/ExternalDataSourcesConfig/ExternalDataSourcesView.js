...
const ExternalDataSourcesView = ({
  ...
  const displayRectype = (rectype) => {
    if (rectype === 1) {
      return <FormattedMessage id="ui-local-kb-admin.settings.externalDataSources.package" />;
    }

    if (rectype === 2) {
      return <FormattedMessage id="ui-local-kb-admin.settings.externalDataSources.title" />;
    }
      ...
      <Row>
        ...
        <Col xs={4}>
          <KeyValue
            ...
            value={displayRectype(value.rectype)}
          />
        </Col>
      </Row>
      <Row>
